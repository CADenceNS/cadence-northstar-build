...
- `NORTHSTAR_OPERATIONS_CHAIN_REQUIREMENT = OWNER PROVIDED / CAPTURED / PLANNED / NOT IMPLEMENTED` in `END_TO_END_DENTAL_LAB_CRM_OPERATIONAL_CHAIN_REQUIREMENTS.md`. It preserves the connected Logistics→Prescription→Product→Case Product→Production→AR→Reconciliation architecture and independent Clinic Supply workflow without authorizing implementation.
- `PP-1A-F6_SCOPE_PROTECTION = UNCHANGED`; this continuity capture does not alter its Product & Pricing source, tests, migrations, Render configuration, PR #42 state, approved NorthStar shell, or Design Studio.
