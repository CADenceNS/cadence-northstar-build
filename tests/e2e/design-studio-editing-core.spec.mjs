...
test('performs numeric transform, bake, save/reopen, and crash recovery', async ({ page }) => {
  ...
  await page.getByLabel('Project name').fill('Editing Persistence'); await page.getByRole('button', { name: 'Save', exact: true }).click(); await expect(page.getByText('Saved · Schema v5')).toBeVisible(); await page.getByRole('button', { name: 'Close' }).click(); await page.getByRole('button', { name: 'Open' }).click(); await page.getByRole('button', { name: /Editing Persistence/ }).click(); await page.getByRole('button', { name: 'Edit', exact: true }).click(); await expect(page.locator('.geometry-comparison')).toContainText('Transform Bake');
