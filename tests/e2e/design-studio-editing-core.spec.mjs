...
test('performs numeric transform, bake, save/reopen, and crash recovery', async ({ page }) => {
  ...
  await page.getByLabel('Project name').fill('Editing Persistence'); await page.getByRole('button', { name: 'Save', exact: true }).click(); await expect(page.getByText('Saved · Schema v6')).toBeVisible(); await page.getByRole('button', { name: 'Close' }).click(); await page.getByRole('button', { name: 'Open' }).click(); await page.getByRole('button', { name: /Editing Persistence/ }).click(); await page.getByRole('button', { name: 'Edit', exact: true }).click(); await expect(page.locator('.geometry-comparison')).toContainText('Transform Bake');
...
test('creates and persists a model-space surface curve through command history', async ({ page }) => {
  ...
  await page.getByLabel('Project name').fill('Curve Persistence'); await page.getByRole('button', { name: 'Save', exact: true }).click(); const savedCurve = await page.evaluate(() => { const key = Object.keys(localStorage).find((value) => value.startsWith('cadence.design-studio.project.')); const stored = JSON.parse(localStorage.getItem(key)); const project = stored.storageFormat ? stored.project : stored; const curve = project.editing.curves[0]; return { objectId: curve.objectId, artifactId: curve.artifactId, first: curve.controlPoints[0] }; }); expect(savedCurve.objectId).toBeTruthy(); expect(savedCurve.artifactId).toBeTruthy(); expect(savedCurve.first[0]).toBeCloseTo(1.25, 5); await page.reload(); await page.getByRole('button', { name: 'Open' }).click(); await page.getByRole('button', { name: /Curve Persistence/ }).click(); await page.getByRole('button', { name: 'Edit', exact: true }).click(); await expect(page.locator('.curve-list')).toContainText(/(?:[2-9]|\d{2,}) controls/); expect(errors).toEqual([]);
});
