...
async function login(page,email){await page.goto('/');await page.evaluate(()=>{localStorage.clear();sessionStorage.clear();});const result=await page.evaluate(async({email,password})=>{const response=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password})});const body=await response.json();return {status:response.status,csrfToken:body.csrfToken??response.headers.get('X-CSRF-Token')??''};},{email,password});expect(result.status).toBe(200);await expect.poll(()=>page.evaluate(async()=> (await fetch('/api/auth/session')).status)).toBe(200);csrfTokens.set(page,result.csrfToken);}
