...
test('patient and case intake full CRUD lifecycle',async({page})=>{
 await login(page);
 ...
 if(!caseNumber)throw new Error('Case number was not rendered.');
 const caseRow=page.getByText(caseNumber,{exact:true}).locator('..');
 await expect(caseRow.getByText('Implant Crown')).toBeVisible();
 await expect(caseRow.getByText(/rush/)).toBeVisible();
 await page.getByLabel(`Attach file to ${caseNumber}`).setInputFiles({name:'prescription.pdf',mimeType:'application/pdf',buffer:Buffer.from('Sprint 4 RX')});
 await expect(caseRow.getByText('rx: prescription.pdf')).toBeVisible();

 await caseRow.getByRole('button',{name:'Edit'}).click();
 await page.getByLabel('Shade').fill('B1');
 await page.getByRole('button',{name:'Update case'}).click();
 await expect(page.getByText(/Shade B1/)).toBeVisible();
 page.once('dialog',dialog=>dialog.accept());
 await page.getByText(caseNumber,{exact:true}).locator('..').getByRole('button',{name:'Delete'}).click();
 await expect(page.getByText(caseNumber,{exact:true})).not.toBeVisible();

 await page.getByRole('button',{name:'Patients'}).click();
 await page.getByLabel('Search patients').fill('SPR4-001');
 const patientRow=page.getByText('SPR4-001',{exact:true}).locator('..');
 await patientRow.getByRole('button',{name:'Edit'}).click();
 await page.getByLabel('Patient status').selectOption('inactive');
 await page.getByRole('button',{name:'Update patient'}).click();
 await page.getByLabel('Patient status filter').selectOption('inactive');
 await expect(page.getByText('Taylor Morgan')).toBeVisible();
 page.once('dialog',dialog=>dialog.accept());
 await page.getByText('SPR4-001',{exact:true}).locator('..').getByRole('button',{name:'Delete'}).click();
 await expect(page.getByText('SPR4-001',{exact:true})).not.toBeVisible();
