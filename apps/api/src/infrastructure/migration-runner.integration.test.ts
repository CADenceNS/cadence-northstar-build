...
await withSchema(async schema=>{
  ...
  assert.deepEqual(result.applied,['0011','0012','0013','0014','0015'],'only missing post-legacy migrations may execute after legacy adoption');
});

await withSchema(async schema=>{
  await applyRaw(schema,11);
  const result=await runMigrations({connectionString,schema});
  assert.deepEqual(result.applied,['0012','0013','0014','0015']);
...
await withSchema(async schema=>{
  ...
  const result=await runMigrations({connectionString,schema});assert.deepEqual(result.applied,['0013','0014','0015']);
...
await withSchema(async schema=>{
  ...
  const result=await runMigrations({connectionString,schema});
  assert.deepEqual(result.applied,['0011','0012','0013','0014','0015'],'an existing 0001–0010 ledger must execute only the missing migrations');
});
