...
try{
 const information=await request('/api/system/information');assert.equal(information.response.status,200);assert.equal(information.body.migrationVersion,'0011');
 ...
 const tenantBEcc=await request('/api/uat/ecc',{headers:{'x-test-tenant':tenantB}});assert.equal(Number(tenantBEcc.body.cases),0,'ECC data must remain tenant-isolated.');
