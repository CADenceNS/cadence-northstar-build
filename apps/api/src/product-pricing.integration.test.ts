...
await pool.query(`INSERT INTO doctors(id,tenant_id,practice_id,first_name,last_name,email,created_at,updated_at) VALUES($1,$2,$3,'Product','Administrator','product.admin@example.test',now(),now()) ON CONFLICT(id) DO UPDATE SET tenant_id=EXCLUDED.tenant_id,practice_id=EXCLUDED.practice_id,deleted_at=NULL`,[doctorId,tenantId,practiceId]);
