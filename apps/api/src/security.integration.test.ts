...
try{
 ...
 const currentPayload=await current.json() as {csrfToken:string;user:User};assert.equal(currentPayload.user.id,user.id);assert.ok(currentPayload.csrfToken.length>=24);assert.notEqual(currentPayload.csrfToken,payload.csrfToken,'session restoration must rotate CSRF state');
 ...
 const staleCsrf=await fetch(`${base}/api/practices`,{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json','X-CSRF-Token':payload.csrfToken},body:'{}'});assert.equal(staleCsrf.status,403,'rotated CSRF state must reject the previous token');
 const allowed=await fetch(`${base}/api/practices`,{method:'POST',headers:{Cookie:cookie,'Content-Type':'application/json','X-CSRF-Token':currentPayload.csrfToken},body:'{}'});assert.equal(allowed.status,201,'authorized mutation with current CSRF must pass');
 const logout=await fetch(`${base}/api/auth/logout`,{method:'POST',headers:{Cookie:cookie,'X-CSRF-Token':currentPayload.csrfToken}});assert.equal(logout.status,204);
 ...
 const auditCount=await durable.pool.query<{count:string}>(`SELECT count(*)::text AS count FROM audit_events WHERE action IN ('authentication.login','authentication.logout','authorization.denied','authorization.csrf')`);assert.ok(Number(auditCount.rows[0]?.count??0)>=6,'security actions must be immutably audited');
