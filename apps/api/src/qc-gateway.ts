...
const internalUpstreamPort=Number(process.env.NORTHSTAR_INTERNAL_UPSTREAM_PORT??4001);
if(!Number.isInteger(externalPort)||externalPort<=0)throw new Error('PORT must be a positive integer.');
if(!Number.isInteger(internalUpstreamPort)||internalUpstreamPort<=0)throw new Error('NORTHSTAR_INTERNAL_UPSTREAM_PORT must be a positive integer.');
if(externalPort===internalUpstreamPort)throw new Error('PORT and NORTHSTAR_INTERNAL_UPSTREAM_PORT must be different.');
...
const app=express();const upstream=`http://127.0.0.1:${internalUpstreamPort}`;const now=()=>new Date().toISOString();
...
installCommunications(app,durable.pool,durable.objects);
    ...
    app.use('/api/cases',(req:SecurityRequest,res,next)=>{const operational=/(?:^|\/)intake-method$|(?:^|\/)intake-routing$|(?:^|\/)(?:hold|release-hold|cancel|files|vendor-packages)(?:\/|$)|(?:^|\/)product-lines\/[^/]+\/fulfillment$/.test(req.path);if(operational&&!['GET','HEAD','OPTIONS'].includes(req.method)&&(!req.identity||!intakeWriters.has(req.identity.role)))return res.status(403).json({error:'Permission denied.'});return next()});
...
app.use('/api',async(req:SecurityRequest,res)=>{if(!req.identity)return res.status(401).json({error:'Authentication required.'});if(req.identity.platformRole==='platform-admin')return res.status(403).json({error:'Platform administrators do not have tenant operational access.'});const headers=new Headers();for(const[key,value]of Object.entries(req.headers)){if(typeof value==='string'&&!['host','content-length','cookie','x-actor-id','x-actor-name','x-northstar-role','x-northstar-tenant','x-northstar-suppress-audit','x-northstar-intake-source',internalTenantContextHeader].includes(key.toLowerCase()))headers.set(key,value)}headers.set(internalTenantContextHeader,issueInternalTenantContext({actorId:req.identity.userId,actorName:req.identity.name,tenantId:req.identity.tenantId,laboratoryRole:req.identity.role,platformRole:'none'}));const hasBody=!['GET','HEAD'].includes(req.method);const response=await fetch(`${upstream}${req.originalUrl}`,{method:req.method,headers,body:hasBody?JSON.stringify(req.body):undefined});res.status(response.status);response.headers.forEach((value,key)=>{if(key.toLowerCase()!=='content-encoding'&&key.toLowerCase()!=='content-length')res.setHeader(key,value)});return res.send(Buffer.from(await response.arrayBuffer()))});
app.use(commercialErrorHandler);
const server=app.listen(externalPort,'0.0.0.0',()=>console.log(`CADence NorthStar secure gateway listening on port ${externalPort}`));
