...
import { initializeCaseOperationsWithClient } from './case-operations.js';
...
async function subtypeId(tenant:string,productId:string){const result=await pool.query<{subtype_id:string}>('SELECT subtype_id FROM product_restoration_subtype_mappings WHERE tenant_id=$1 AND product_id=$2 AND active LIMIT 1',[tenant,productId]);assert.ok(result.rows[0],`Missing restoration subtype mapping for ${productId}.`);return result.rows[0]!.subtype_id;}
const fixedConfiguration={shadeSystem:{code:'VITA_CLASSICAL'},shade:{code:'A2'},marginDesign:{code:'CHAMFER'},prepProvided:{code:'PROVIDED'},opposingProvided:{code:'PROVIDED'},biteProvided:{code:'PROVIDED'}};
...
try{
  ...
  const zirconiaSubtype=await subtypeId(tenantId,zirconia.id),unpricedSubtype=await subtypeId(tenantId,unpriced.id),dentureSubtype=await subtypeId(tenantId,denture.id),abutmentSubtype=await subtypeId(tenantId,abutment.id);
  const fixed=await prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]});
  const legacyPracticeFixed=await prepareCaseProductLines(pool,tenantId,{practiceId:legacyRepositoryPracticeId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]});
  await assert.rejects(prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:unpriced.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:unpricedSubtype}}]}),/PRICE NOT CONFIGURED/);
  ...
  const removable=await prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:denture.id,categoryCode:'REM',quantity:1,arch:'both',configuration:{},options:{restorationSubtypeId:dentureSubtype}},{productId:abutment.id,categoryCode:'IMP',quantity:1,arch:'upper',toothNumbers:[8],componentCount:1,configuration:{},options:{restorationSubtypeId:abutmentSubtype}}]});
  assert.equal(removable.lines.length,2);assert.equal(removable.subtotal,1825);assert.equal(removable.turnaroundBusinessDays,14);assert.equal(removable.calculatedDueDate,'2026-08-28');
  ...
  const multipleClosures=await prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]});
  ...
  const noClosures=await prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]});
  assert.equal(noClosures.calculatedDueDate,'2026-08-21');
  const dstBoundary=await prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-10-23',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]});
  ...
  const yearBoundary=await prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-12-18',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]});
  assert.equal(yearBoundary.calculatedDueDate,'2027-01-05');
  await assert.rejects(prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'REM',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]}),/selected restoration category/);
  await assert.rejects(prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:otherZirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]}),/Active product not found/);
  ...
  await assert.rejects(prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}},{productId:denture.id,categoryCode:'REM',quantity:1,arch:'upper',configuration:{},options:{restorationSubtypeId:dentureSubtype}}]}),/incompatible product stack/);
  const client=await pool.connect();try{await client.query('BEGIN');await client.query("INSERT INTO repository_documents(tenant_id,entity_type,entity_id,payload) VALUES($1,'case',$2,$3::jsonb)",[tenantId,caseId,JSON.stringify({id:caseId,practiceId,receivedDate:'2026-08-07'})]);await savePreparedCaseProductLines(client,tenantId,caseId,fixed.lines,'case-builder-test');await initializeCaseOperationsWithClient(client,{tenantId,actorId:'case-builder-test',actorName:'Case Builder Test'},caseId,'HYBRID',fixed.calculatedDueDate);await client.query('COMMIT');}catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}
  ...
  const operational=await pool.query<{intake_method:string;current_calculated_due_date:string}>('SELECT intake_method,current_calculated_due_date::text FROM case_intake_profiles WHERE tenant_id=$1 AND case_id=$2',[tenantId,caseId]);assert.deepEqual(operational.rows[0],{intake_method:'HYBRID',current_calculated_due_date:'2026-08-21'});
  await pool.query('UPDATE product_catalog SET active=false WHERE tenant_id=$1 AND id=$2',[tenantId,zirconia.id]);
  await assert.rejects(prepareCaseProductLines(pool,tenantId,{practiceId,receivedDate:'2026-08-07',productLines:[{productId:zirconia.id,categoryCode:'FIX',quantity:1,arch:'upper',toothNumbers:[8],configuration:fixedConfiguration,options:{restorationSubtypeId:zirconiaSubtype}}]}),/Active product not found/);
  console.log('PP-1B-F2A1 Case Builder integration tests passed.');
}finally{try{await pool.query('DELETE FROM case_product_line_lineage WHERE tenant_id=$1',[tenantId]);await pool.query('DELETE FROM case_product_lines WHERE tenant_id=$1 AND case_id=$2',[tenantId,caseId]);await pool.query('DELETE FROM case_product_tat_overrides WHERE tenant_id=$1 AND case_id=$2',[tenantId,caseId]);await pool.query('DELETE FROM tenant_business_closure_days WHERE tenant_id IN ($1,$2)',[tenantId,otherTenantId]);await pool.query(`DELETE FROM case_product_line_case_entities WHERE tenant_id=$1 AND case_entity_type='case' AND case_id=$2`,[tenantId,caseId]);}finally{await pool.end();}}
