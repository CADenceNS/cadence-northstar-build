...
try{
 ...
 const doctorPreference=await request('/api/intake/admin/doctor-preferences','POST',{doctorId:'doctor-1',practiceId:'practice-1',preferredRoute:'hybrid',preferredOutsourcePartner:'Hybrid Partner',defaults:{material:'Zirconia',marginDesign:'Chamfer',contacts:'Normal',occlusion:'Light'}});assert.equal(doctorPreference.response.status,201);
 const schedule=await request('/api/intake/admin/pricing-schedules','POST',{scheduleType:'contract',name:'Practice Contract Foundation',practiceId:'practice-1',priority:10});assert.equal(schedule.response.status,201);assert.equal('price' in schedule.payload,false);
 ...
 const doctorResolved=await request(`/api/intake/submissions/${submissionId}/route`,'POST',{});assert.equal(doctorResolved.response.status,200);assert.equal(doctorResolved.payload.route,'hybrid');assert.equal(doctorResolved.payload.precedence_source,'doctor-profile');
 const deactivateDoctor=await request(`/api/intake/admin/doctor-preferences/${doctorPreference.payload.id}`,'DELETE');assert.equal(deactivateDoctor.response.status,204);
 const practiceResolved=await request(`/api/intake/submissions/${submissionId}/route`,'POST',{});assert.equal(practiceResolved.response.status,200);assert.equal(practiceResolved.payload.route,'outsourced');assert.equal(practiceResolved.payload.precedence_source,'practice-profile');
 const deactivatePractice=await request('/api/intake/admin/practice-routing/practice-1','DELETE');assert.equal(deactivatePractice.response.status,204);
 const tenantResolved=await request(`/api/intake/submissions/${submissionId}/route`,'POST',{});assert.equal(tenantResolved.response.status,200);assert.equal(tenantResolved.payload.route,'internal');assert.equal(tenantResolved.payload.precedence_source,'tenant-default');
 ...
 const detail=await request(`/api/intake/submissions/${submissionId}`);assert.equal(detail.response.status,200);assert.equal(detail.payload.submission.status,'operational-case-created');assert.ok(detail.payload.history.length>=9);assert.ok(detail.payload.attachments.some((item:{purpose:string})=>item.purpose==='upper-arch'));assert.ok(detail.payload.attachments.some((item:{purpose:string})=>item.purpose==='prescription-production'));assert.equal(detail.payload.products[0].frozen_at!==null,true);
 ...
 assert.ok(audits.some(item=>item.action==='intake.submission.created'));assert.ok(audits.some(item=>item.action==='intake.prescription.changed'));assert.ok(audits.some(item=>item.action==='intake.submission.accepted'));assert.ok(audits.some(item=>item.action==='pricing-schedule.created'));
