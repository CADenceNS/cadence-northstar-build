...
export interface MeshData { positions: number[]; normals: number[]; indices: number[]; bounds: { min: Vec3; max: Vec3 }; }
...
export interface SceneObject {
  ...
  transform: Transform; material: MaterialState; selected: boolean;
  metadata: Record<string, string | number | boolean | null>;
}
...
export function createProject(name = 'Untitled Project'): DesignProject {
  ...
  return { schemaVersion: 1, id: crypto.randomUUID(), name, createdAt: now, updatedAt: now, camera: structuredClone(DEFAULT_CAMERA), settings: structuredClone(DEFAULT_SETTINGS), scene: [], artifacts: [] };
}
...
export class SceneManager {
  ...
  addFromArtifact(artifact: ArtifactRecord, kind: ArtifactKind = inferArtifactKind(artifact.sourceName)): SceneObject {
    const object: SceneObject = { id: crypto.randomUUID(), name: artifact.sourceName, type: kind, artifactId: artifact.id, visible: true, isolated: false, transform: identityTransform(), material: defaultMaterial(), selected: false, metadata: { sourceFormat: artifact.sourceFormat, checksum: artifact.checksum } };
...
export function migrateProject(input: unknown): DesignProject {
  ...
  return { schemaVersion: 1, id: candidate.id, name: candidate.name, createdAt: candidate.createdAt ?? new Date().toISOString(), updatedAt: candidate.updatedAt ?? new Date().toISOString(), camera: { ...structuredClone(DEFAULT_CAMERA), ...(candidate.camera ?? {}) }, settings: { ...structuredClone(DEFAULT_SETTINGS), ...(candidate.settings ?? {}) }, scene: Array.isArray(candidate.scene) ? structuredClone(candidate.scene) : [], artifacts: Array.isArray(candidate.artifacts) ? structuredClone(candidate.artifacts) : [] };
}

export function inferArtifactKind(name: string): ArtifactKind {
  const value = name.toLowerCase(); if (value.includes('upper') || value.includes('maxilla')) return 'upper'; if (value.includes('lower') || value.includes('mandible')) return 'lower'; if (value.includes('opposing') || value.includes('antagonist')) return 'opposing'; if (value.includes('bite') || value.includes('occlusion')) return 'bite'; return 'unknown';
}
...
function parseStl(bytes: Uint8Array): MeshData {
  ...
  if (!positions.length) throw new Error('STL contains no triangles'); return finalizeMesh(positions, normals, indices);
}
...
function parseObj(text: string): MeshData {
  ...
  for (const rawLine of text.split(/\r?\n/)) { const line = rawLine.trim(); if (line.startsWith('v ')) vertices.push(tuple(line.slice(2))); if (line.startsWith('f ')) { const refs = line.slice(2).trim().split(/\s+/).map((token) => Number(token.split('/')[0])); for (let i = 1; i < refs.length - 1; i += 1) { const triangle = [resolveObjVertex(vertices, refs[0]), resolveObjVertex(vertices, refs[i]), resolveObjVertex(vertices, refs[i + 1])] as [Vec3, Vec3, Vec3]; const normal = faceNormal(...triangle); triangle.forEach((vertex) => { positions.push(...vertex); normals.push(...normal); indices.push(indices.length); }); } } }
  if (!positions.length) throw new Error('OBJ contains no faces'); return finalizeMesh(positions, normals, indices);
}
...
function parsePly(text: string): MeshData {
  ...
  if (!positions.length) throw new Error('PLY contains no faces'); return finalizeMesh(positions, normals, indices);
}
...
function resolveObjVertex(vertices: Vec3[], index: number): Vec3 { const value = vertices[index < 0 ? vertices.length + index : index - 1]; if (!value) throw new Error('OBJ face references missing vertex'); return value; }
