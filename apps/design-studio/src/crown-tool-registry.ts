...
const core: Seed[] = [
  ...
  direct('crown.contact-optimize', 'Dynamic proximal contact correction', 'contact', 'Move local proximal crown vertices to governed distance targets while preserving the approved margin', 'Adjacent mesh, target and unlocked side', 'Bounded nearest-surface displacement over non-margin outer support with deterministic backtracking to the largest topology-valid step', analysisEvidence, browserEvidence, 'Lock conflict, empty editable non-margin support, infeasible governed target reported as best-effort/QC failure, or corrupt output'),
