    ...
    <div className="workspace">
      <aside className="scene-panel" aria-label="Production scene tree">
        ...
        <div className="scene-list">
          {scene.map((object) => {
            const artifact = artifactMap.get(object.artifactId); const caseScan = caseScanSet.scans.find((scan) => scan.sceneObjectId === object.id); const role = caseScan ? SCAN_ROLES.find((item) => item.value === caseScan.assignedRole)?.label ?? caseScan.assignedRole : DENTAL_ROLES.find((item) => item.value === object.type)?.label ?? object.type;
