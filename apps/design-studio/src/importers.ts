...
export class ManagedMeshImporter implements IImporter {
  ...
  async import(request: ImportRequest, existing: ArtifactRecord[]): Promise<ImportResult> {
    ...
    const checksum = await checksumOf(bytes);
    const checksumMetric = {
      name: 'import.checksum', durationMs: performance.now() - checksumStart, startedAt: new Date().toISOString(),
    ...
    const parseMetric = {
      name: 'import.parse', durationMs: performance.now() - parseStart, startedAt: new Date().toISOString(),
      metadata: { format, triangles: mesh.indices.length / 3, bytes: bytes.byteLength },
    };
    ...
    const artifact: ArtifactRecord = Object.freeze({
      ...
      units: request.units ?? inferUnits(request.file.name),
      orientation: request.orientation ?? 'source',
      metadata: Object.freeze({
        vertexCount: mesh.positions.length / 3,
        triangleCount: mesh.indices.length / 3,
        format,
        sourceLastModified: request.file.lastModified,
        sourceMimeType: request.file.type || 'application/octet-stream',
      }),
      history: Object.freeze([{ at: now, action: 'imported', detail: `${format.toUpperCase()} source preserved by SHA-256 checksum` }]),
      mesh: Object.freeze({
        positions: Object.freeze([...mesh.positions]) as unknown as number[],
        normals: Object.freeze([...mesh.normals]) as unknown as number[],
        indices: Object.freeze([...mesh.indices]) as unknown as number[],
        bounds: Object.freeze({ min: Object.freeze([...mesh.bounds.min]), max: Object.freeze([...mesh.bounds.max]) }) as ArtifactRecord['mesh']['bounds'],
      }),
    }) as ArtifactRecord;

    const totalMetric = {
      name: 'import.total', durationMs: performance.now() - startedAt, startedAt: new Date().toISOString(),
      metadata: { format, bytes: bytes.byteLength, triangles: mesh.indices.length / 3 },
    };
...
function extensionOf(name: string): MeshFormat | null {
  const extension = name.split('.').pop()?.toLowerCase();
  return extension && SUPPORTED.has(extension as MeshFormat) ? extension as MeshFormat : null;
}

function inferUnits(name: string): ArtifactRecord['units'] {
  const value = name.toLowerCase();
  if (/(^|[_\-.])cm([_\-.]|$)/.test(value)) return 'cm';
  if (/(^|[_\-.])m([_\-.]|$)/.test(value)) return 'm';
  return 'mm';
}

function validateMesh(positions: number[], normals: number[], indices: number[]): void {
  if (!positions.length || positions.length % 3) throw new Error('Mesh positions are invalid.');
  if (normals.length !== positions.length) throw new Error('Mesh normals do not match vertex data.');
  if (!indices.length || indices.length % 3) throw new Error('Mesh does not contain complete triangles.');
  if (![...positions, ...normals].every(Number.isFinite)) throw new Error('Mesh contains non-finite geometry values.');
  const vertexCount = positions.length / 3;
  if (indices.some((index) => !Number.isInteger(index) || index < 0 || index >= vertexCount)) throw new Error('Mesh contains an invalid index.');
}
