...
function inferUnits(name: string): ArtifactRecord['units'] { const value = name.toLowerCase(); if (/(^|[_\-.])mm([_\-.]|$)/.test(value)) return 'mm'; if (/(^|[_\-.])cm([_\-.]|$)/.test(value)) return 'cm'; if (/(^|[_\-.])m([_\-.]|$)/.test(value)) return 'm'; return 'unknown'; }
