...
export type GoldenCrownFamily = 'maxillary-central-incisor' | 'maxillary-canine' | 'maxillary-first-premolar' | 'maxillary-first-molar' | 'mandibular-incisor' | 'mandibular-premolar' | 'mandibular-first-molar';
...
const TOOTH_BY_FAMILY: Record<GoldenCrownFamily, number> = {
  ...
  'mandibular-incisor': 25, 'mandibular-premolar': 20, 'mandibular-first-molar': 19,
};
...
export function goldenCrown(family: GoldenCrownFamily, materialId: CrownMaterialId = 'zirconia-monolithic', resolution: { radialSegments?: number; surfaceRings?: number } = {}): GoldenCrownCase {
  ...
  const base: CrownGenerationInput = { requestId: `golden-${family}-base`, preparationId: `preparation-${family}`, preparationArtifactId: preparation.artifact.id, preparationMesh: preparation.artifact.mesh, marginPoints: preparation.trueMargins[0], insertionAxis: [0, 0, 1], toothNumber: String(toothNumber), materialProfileId: materialId, parameters, adjacentMeshes: [] };
  const proposal = generateCrownProposal(base); const referenceMesh = inflatedReference(proposal.mesh, 0.12); const adapted = generateCrownProposal({ ...base, requestId: `${base.requestId}-reference`, referenceMesh }); const bounds = adapted.mesh.bounds; const contact = 0.025; const sideWidth = 4; const verticalPadding = 2;
  ...
  const input: CrownGenerationInput = { ...base, requestId: `golden-${family}`, adjacentMeshes: [{ objectId: `object-${mesial.id}`, side: 'mesial', mesh: mesial.mesh }, { objectId: `object-${distal.id}`, side: 'distal', mesh: distal.mesh }], antagonist: { objectId: `object-${antagonist.id}`, mesh: antagonist.mesh }, referenceMesh: reference.mesh };
