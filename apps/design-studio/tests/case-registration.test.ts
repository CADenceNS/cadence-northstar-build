...
describe('registration reports and project persistence', () => {
  ...
  it('preserves scan transforms, XYZ, results, reports and corrections through save/reopen and recovery', async () => {
    ...
    expect(opened.schemaVersion).toBe(4); expect(opened.caseScanSet).toEqual(project.caseScanSet); expect(opened.registrationReports).toEqual([report]); expect(opened.history.at(-1)).toEqual(historyEntry);
