...
describe('validation reports and project persistence', () => {
  ...
  it('preserves validation reports, measurements and scene state across save and reopen', async () => {
    ...
    expect(opened.validationReports).toEqual([report]); expect(opened.measurements).toEqual(project.measurements); expect(opened.history).toEqual([historyEntry]); expect(opened.schemaVersion).toBe(4);
  });
