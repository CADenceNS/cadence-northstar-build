...
import { analyzeOcclusion, analyzeProximalContact, autoThickenCrown, optimizeProximalContact, optimizeStaticOcclusion } from '../src/crown-analysis';
...
function restoration(fixture: ReturnType<typeof goldenCrown>): RestorationRecord {
  const now = new Date(0).toISOString(); return { id: 'restoration', preparationId: fixture.input.preparationId, preparationVersionId: 'segmentation', approvedMarginVersionId: 'margin', insertionAxisAnalysisId: 'axis', toothNumber: String(fixture.toothNumber), morphologyId: fixture.input.parameters.morphologyId, morphologyVersion: 'CADENCE-MORPHOLOGY-1.0.0', materialProfileId: fixture.input.materialProfileId, artifactId: 'crown', sceneObjectId: 'crown-object', parameters: fixture.input.parameters, locks: { margin: true, intaglio: false, mesialContact: false, distalContact: false, occlusion: false, anatomy: false }, topologyMap: fixture.result.topologyMap, thickness: fixture.result.thickness, cementSpace: fixture.result.cementSpace, seating: fixture.result.seating, mesialContact: fixture.result.mesialContact, distalContact: fixture.result.distalContact, occlusion: fixture.result.occlusion, contour: fixture.result.contour, qcResultIds: [], activeQcResultId: null, versionIds: ['v1'], activeVersionId: 'v1', exportRecordIds: [], approvalState: 'PROPOSAL_GENERATED', approvedAt: null, approvedBy: null, createdAt: now, updatedAt: now };
}
