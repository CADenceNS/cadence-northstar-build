...
describe('editing persistence and coverage registry', () => {
  it('persists selections, curves, tool settings, lineage, save/reopen, and recovery', () => {
    ...
    const store = new ProjectStore(); const saved = store.save(project); const opened = store.open(saved.id); expect(opened.schemaVersion).toBe(4); expect(opened.editing).toEqual(project.editing); store.autoSave(opened); expect(store.recover()?.editing).toEqual(project.editing);
  });
  it('migrates older projects to independently owned editing state', () => { Object.defineProperty(globalThis, 'localStorage', { configurable: true, value: new MemoryStorage() }); const project = createProject('Old'); const old = { ...project, schemaVersion: 3 }; delete (old as Partial<typeof project>).editing; localStorage.setItem(`cadence.design-studio.project.${project.id}`, JSON.stringify(old)); const opened = new ProjectStore().open(project.id); expect(opened.schemaVersion).toBe(4); expect(opened.editing.schemaVersion).toBe(1); expect(opened.editing.curves).toHaveLength(0); });
