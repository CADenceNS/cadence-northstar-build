...
describe('production cutting, Boolean, and self-intersection foundation', () => {
  ...
  it('trims actual mesh faces by a closed model-space curve', () => { const source = grid(10); const curve = [[2, 2], [8, 2], [8, 8], [2, 8]].map(([x, y]) => [x, y, Math.sin(x * 0.1) * Math.cos(y * 0.1)] as [number, number, number]); const result = trimByClosedCurve(source, curve, true, true); expect(result.faces.length).toBeGreaterThan(0); expect(result.positions.some(([x, y]) => !Number.isInteger(x) || !Number.isInteger(y))).toBe(true); expect(buildTopology(result).nonManifoldEdges).toHaveLength(0); });
