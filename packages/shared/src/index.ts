...
export interface Practice {
  id: string; accountNumber: string; practiceName: string; status: EntityStatus; phone: string; email: string;
  address: string; city: string; state: string; postalCode: string; taxExempt: boolean; scannerType: string;
  officeManager: Contact; notes: string; communicationHistory: CommunicationEntry[]; createdAt: string; updatedAt: string;
}

export interface Doctor {
  id: string; practiceId: string; firstName: string; lastName: string; specialty: string; email: string; phone: string;
  status: EntityStatus; notes: string; communicationHistory: CommunicationEntry[]; createdAt: string; updatedAt: string;
}
...
export interface Patient {
  id: string; practiceId: string; doctorId: string; patientReference: string; firstName: string; lastName: string;
  dateOfBirth: string; status: EntityStatus; notes: string; createdAt: string; updatedAt: string;
}
export type PatientInput = Omit<Patient, 'id' | 'createdAt' | 'updatedAt'>;

export interface CaseAttachment {
  id: string; caseId: string; kind: AttachmentKind; fileName: string; mimeType: string; size: number; contentBase64: string; uploadedAt: string;
}

export interface ClinicalCase {
  id: string; caseNumber: string; patientId: string; practiceId: string; doctorId: string; status: CaseStatus;
  toothNumbers: number[]; arch: ArchSelection; restoration: string; material: string; shade: string; stumpShade: string;
  rushPriority: RushPriority; receivedDate: string; dueDate: string; prescriptionNotes: string; attachments: CaseAttachment[];
...
export interface DashboardSnapshot { generatedAt: string; casesReceivedToday: number; casesDueToday: number; casesAtRisk: number; casesInQc: number; shipmentsReady: number; monthRevenue: number; activeDoctors: number; activePractices: number; activePatients: number; openCases: number; rushCases: number; }
