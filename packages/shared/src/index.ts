...
export type ProductCategoryCode = 'FIX' | 'REM' | 'IMP' | 'ORT' | 'SLP' | 'DIA' | 'SPL' | 'AUX';
export type ProductPricingBasis = 'PER_UNIT' | 'PER_TOOTH' | 'PER_PRODUCT' | 'PER_ARCH' | 'PER_CASE' | 'PER_COMPONENT' | 'PER_STAGE' | 'QUANTITY_BASED';
...
export interface CaseProductLineInput { productId:string; categoryCode:ProductCategoryCode; quantity:number; arch?:string|null; toothNumbers?:number[]; unitCount?:number|null; stageCount?:number|null; componentCount?:number|null; configuration?:Record<string,unknown>; options?:Record<string,unknown>; notes?:string; }
export interface CaseProductLineSnapshot extends CaseProductLineInput { id:string; lineNumber:number; productSku:string; productName:string; productDescription:string; familyCode:string; pricingBasis:ProductPricingBasis; unitPrice:number; lineTotal:number; turnaroundBusinessDays:number|null; priceVersionId:string; }
export interface CaseDueDateOverrideInput { revisedDueDate:string; reason:string; }
export interface CaseDueDateOverride extends CaseDueDateOverrideInput { authorizedBy:string; authorizedAt:string; }
export interface ClinicalCase { id:string; caseNumber:string; patientId:string; practiceId:string; doctorId:string; status:CaseStatus; toothNumbers:number[]; arch:ArchSelection; restoration:string; material:string; shade:string; stumpShade:string; rushPriority:RushPriority; receivedDate:string; dueDate:string; prescriptionNotes:string; attachments:CaseAttachment[]; createdAt:string; updatedAt:string; intakeMethod?:'DIGITAL'|'PHYSICAL'|'HYBRID'; lifecycleState?:'ACTIVE'|'ON_HOLD'|'RELEASED'|'CANCELLED'; currentCalculatedDueDate?:string|null; caseRelationship?:CaseRelationship; journeyAvailable?:boolean; rootCaseId?:string; parentCaseId?:string|null; remakeRepairReasonId?:string|null; continuationStageId?:string|null; continuationOperationalState?:ContinuationOperationalState|null; continuationBillingPolicyId?:string|null; responsibility?:CaseJourneyResponsibility|CaseJourneyResponsibilityInput|null; restorationCategory?:ProductCategoryCode|null; productLines?:CaseProductLineInput[]|CaseProductLineSnapshot[]; calculatedDueDate?:string|null; dueDateOverride?:CaseDueDateOverrideInput|CaseDueDateOverride|null; }
